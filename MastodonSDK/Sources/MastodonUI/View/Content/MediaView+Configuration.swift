...
extension MediaView {
    public static func configuration(status: Mastodon.Entity.Status) -> [MediaView.Configuration] {
...
//        let status: StatusCompatible = status.reblog ?? status
        ...
        let configurations = attachments.enumerated().map { (idx, attachment) -> MediaView.Configuration in
            ...
            configuration.load()
            configuration.isReveal = status.sensitive == true ? status.sensitiveToggled : true
