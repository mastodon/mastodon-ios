...
3. Set the `Codegen` to  `Manual/None`. We use `Sourcery` generates the template code.
4. Create the `Entity.swift` file and declare the properties and relationships.
...
The iOS Share Extension allows users to share links or media from other apps. The app uses the same implementation for the main app and the share extension. Then different is less available memory for extension so maybe some memory bounded task could crash the app. (Please file the issue)
